...
Direction pendingDirection = RIGHT; // Thêm biến pendingDirection
    ...
    if (event->type == SDL_EVENT_KEY_DOWN && !gameend) {
        switch (event->key.scancode) {
            ...
            case SDL_SCANCODE_DOWN: {
                if (direction != UP) {
                    pendingDirection = DOWN; // Chỉ cập nhật pendingDirection
                }
                break;
            }
            case SDL_SCANCODE_UP: {
                if (direction != DOWN) {
                    pendingDirection = UP; // Chỉ cập nhật pendingDirection
                }
                break;
            }
            case SDL_SCANCODE_LEFT: {
                if (direction != RIGHT) {
                    pendingDirection = LEFT; // Chỉ cập nhật pendingDirection
                }
                break;
            }
            case SDL_SCANCODE_RIGHT: {
                if (direction != LEFT) {
                    pendingDirection = RIGHT; // Chỉ cập nhật pendingDirection
                }
                break;
            }
            default: {
    ...
    if (started == true) {
        ...
        if (!gameend) {
            ...
            if (currentime - lastime >= timedelay) {
                lastime = currentime;
                direction = pendingDirection; // Cập nhật direction từ pendingDirection
        ...
        if (gameend == true) {
            ...
            if (click == true) {
                ...
                direction = RIGHT;
                pendingDirection = RIGHT; // Đặt lại pendingDirection khi restart
                countf = 0;
                countf2 = 1;
                gameend = false;
                click = false;
